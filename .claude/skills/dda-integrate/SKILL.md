...
**Note**: The Effector phase is provider-agnostic — it receives a `DifficultyProfile` regardless of whether the values were generated by Gemini, Claude, or rule-based logic. No provider-specific code is needed here.
...
- `configure-mapping <variable-name>` — Interactively choose how a variable should map to game systems
- `list-mappings` — Show current variable-to-system mappings from config or defaults
...
When `configure-mapping <variable-name>` is invoked:

1. **Load existing config** from `Assets/Scripts/RedRunner/DDA/dda_config.json` if it exists
2. **Show current mapping** for the variable (or "not yet configured")
3. **Offer choices**:
   - For runSpeed/jumpStrength: "Add setter to RedCharacter", "Use reflection", "Manual wiring"
   - For block probabilities: "Identify by enemy type", "Manual tag blocks", "Custom filter"
   - For coinDensity: "Scale coin children", "Add density parameter to blocks", "Custom"
   - For platformHeightVariance: "Modify TerrainGenerator.CreateBlock", "Custom"
4. **Guide implementation** based on choice
5. **Update config file** with the chosen mapping

When `list-mappings` is invoked, show a table:
```
| Variable                 | Maps To                        | Method        | Status      |
|--------------------------|--------------------------------|---------------|-------------|
| runSpeed                 | RedCharacter.m_RunSpeed        | Setter        | Wired       |
| jumpStrength             | RedCharacter.m_JumpStrength    | Setter        | Wired       |
| enemyDensity             | Block probabilities (Enemy)    | Filter        | Not wired   |
| coinDensity              | Coin spawn in blocks           | Custom        | Not wired   |
...
```
...
6. **Config file not found**: If `dda_config.json` doesn't exist, use default mappings and offer to create config
